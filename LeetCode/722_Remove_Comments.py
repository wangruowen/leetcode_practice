...
class Solution:
    def removeComments(self, source):
        ...
        for each_line in source:
            if within_block_comment:
                block_comment_end = each_line.find("*/")
                if block_comment_end >= 0:
                    new_line = incomp_line
                    offset = len(new_line)
                    new_line += each_line[block_comment_end + 2:]
                    within_block_comment = False
                else:
                    continue
            else:
            ...
            while new_line.find("//", offset) >= 0 or new_line.find("/*", offset) >= 0:
                line_comment_start = new_line.find("//", offset)
                if line_comment_start < 0:
                    line_comment_start = float('inf')
                block_comment_start = new_line.find("/*", offset)
                if block_comment_start < 0:
                    block_comment_start = float('inf')
                if line_comment_start < block_comment_start:
                    new_line = new_line[:line_comment_start]
                    break
                if block_comment_start < line_comment_start:
                    incomp_line = new_line[:block_comment_start]
                    block_comment_end = new_line.find("*/", block_comment_start + 2)
                    if block_comment_end > 0:
                        offset = len(incomp_line)
                        incomp_line += new_line[block_comment_end + 2:]
                        new_line = incomp_line
                    else:
                        within_block_comment = True
                        break

            if not within_block_comment and len(new_line) > 0:
